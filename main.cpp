  ...
  for (int i = 0; i < flight_groups.size(); i++) {
    ...
    for (int j = 0; j < graph->path_list.size(); j++) {
      for (int k = 0; k < graph->path_list[j].size() - 1; k++) {
        if ((graph->path_list[j][k + 1]->time_arrive -
             graph->path_list[j][k]->time_go) < 45) {
          side_stop++;
        }
      }
    }
    ...
    for (int j = 0; j < graph->path_list.size(); j++) {
      for (int k = 0; k < graph->path_list[j].size(); k++) {
        outFile << graph->path_list[j][k]->id << " "
                << graph->path_list[j][k]->time_arrive << "  "
                << graph->path_list[j][k]->time_go << "  ";
      }
      outFile << endl;
    }
    outFile << endl;
