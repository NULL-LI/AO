...
void Solution1::getWideCombine(){
    ...
    int path_num=graph->path_list.size();
	printf("pairs:%4d path:%4d size:%4d\n",graph->path_num,path_num,flight_wdi_wid.size());
...
void Solution1::getNarrowCombine(){
    ...
    int path_num=graph->path_list.size();
	printf("pairs:%4d path:%4d size:%4d\n",graph->path_num,path_num,flight_di_id_ii.size());
	...
	printf("pairs:%4d path:%4d size:%4d\n",graph->path_num,path_num,flight_groups[0].size());

    shared_ptr<struct FlightGroupStruct > flight_group0(new struct FlightGroupStruct);
    flight_group0->flight_paths.assign(graph->path_list.begin(),graph->path_list.end());
...
//2
    ...
    path_num=graph->path_list.size();
	printf("pairs:%4d path:%4d size:%4d\n",graph->path_num,path_num,flight_groups[2].size());

    shared_ptr<struct FlightGroupStruct > flight_group2(new struct FlightGroupStruct);
    flight_group2->flight_paths.assign(graph->path_list.begin(),graph->path_list.end());
